...
public record CreateEventResponse(string Id, ChannelResponse Channel)
{
    public static CreateEventResponse FromEntity(Event ev, ChannelResponse channel) => new(
        ev.Id, channel);
}
...
public static class CreateEventEndpoint
{
    ...
    {
        events.MapPost("/", [Authorize][Roles((int)Perms.HandleEvent)] async (CreateEventCommand command, CreateEventRequest req, TokenContext token) =>
        {
